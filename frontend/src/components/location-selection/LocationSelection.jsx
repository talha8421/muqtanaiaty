...
// 2. This is the magic fix: Load Leaflet ONLY in the browser
const MapContainer = dynamic(() => import("react-leaflet").then((mod) => mod.MapContainer), { ssr: false });
const TileLayer = dynamic(() => import("react-leaflet").then((mod) => mod.TileLayer), { ssr: false });
const Marker = dynamic(() => import("react-leaflet").then((mod) => mod.Marker), { ssr: false });
const Popup = dynamic(() => import("react-leaflet").then((mod) => mod.Popup), { ssr: false });

// Also need to import the CSS for Leaflet or the map will look broken
...
function LocationSelection() {
  return (
    <div className="flex flex-col gap-2 w-full items-center justify-center">
      <Dialog>
        <DialogTrigger className={"w-full flex items-center justify-center"}>
          <div className="w-[60%] flex bg-moonstone p-2.75 text-white items-center justify-center rounded-[8px] font-medium text-[14px] h-10.25 border-2 border-moonstone gap-1">
            <Location size={18} />
            <h2 className="whitespace-nowrap">Near Me</h2>
          </div>
        </DialogTrigger>
      ...
      <Dialog>
        <DialogTrigger className={"w-full flex items-center justify-center"}>
          <div className="w-[60%] flex bg-white p-2.75 text-moonstone items-center justify-center rounded-[8px] font-medium text-[14px] border-2 border-moonstone box-border">
            <Location />
            <h2 className="whitespace-nowrap">Enter a different address</h2>
          </div>
        </DialogTrigger>
      ...
      <DialogHeader>
        <DialogTitle>
          <div className="flex flex-col gap-3 items-center justify-center ">
            ...
            <div className="h-80 md:h-120 w-full bg-red-100">
              {/* The Map components now only render on the client */}
              <MapContainer 
                center={position} 
                zoom={13} 
