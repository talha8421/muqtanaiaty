...
function LoginPage() {
  const router = useRouter();
  const searchParams = useSearchParams(); // Hook to get current URL params

  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [keepLoggedIn, setKeepLoggedIn] = useState(false);

  // Logic: Get current role to pass it to the next page
  ...
  function handleCreateAccount() {
    // If a role exists, append it to the signup route
    const path = role ? `/sign-up?role=${role}` : '/sign-up';
    router.push(path);
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-10 md:gap-16.5 px-4 pt-35 pb-8 md:py-20">
      ...
      <form action="" className=" w-full flex flex-col gap-2.5" onSubmit={(e) => e.preventDefault()}>
        <div className="flex flex-col gap-5">
          <InputField
            title={"Email"}
            prefixIcon={<EmailIcon />}
        ...
        <div className="flex justify-between items-center">
          ...
          >
            {keepLoggedIn ? <CheckboxFilled /> : <CheckboxEmpty />}
            <p className="text-[14px]">Keep me logged in</p>
          </div>
          <p className="text-[14px] font-medium text-moonstone cursor-pointer" onClick={handleForgetPassword}> 
            Forgot password ?
          </p>
